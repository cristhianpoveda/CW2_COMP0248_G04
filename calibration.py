...
images = glob.glob('./combined_calibration_images/*.png')  # Adjust the path as needed
...
for fname in images:
    ...
    if ret:
        ...
        cv2.waitKey(500)
        output_fname = fname.replace('.png', '_corners_detected.png')  # Adjust extension if necessary
...
img = cv2.imread('./combined_calibration_images/seq2_1772626724554358268_corners_detected.png')  # Replace with an image to undistort
# img = cv2.cvtColor()
h, w = img.shape[:2]
